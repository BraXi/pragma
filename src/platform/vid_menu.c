...
static void BrightnessCallback( void *s )
{
	...
	if ( stricmp( r_renderer->string, "soft" ) == 0 )
	{
...
static void ApplyChanges( void *unused )
{
	...
	*/
	...
	{
	case REF_SOFT:
		Cvar_Set( "r_renderer", "soft" );
		break;
	case REF_OPENGL:
		Cvar_Set( "r_renderer", "gl" );
	...
	** has been modified
	*/
	if ( stricmp( r_renderer->string, "gl" ) == 0 )
	{
		if ( r_gamma->modified )
		{
...
void VID_MenuInit( void )
{
	...
	if ( strcmp( r_renderer->string, "soft" ) == 0 )
	{
		s_current_menu_index = SOFTWARE_MENU;
		s_ref_list[0].curvalue = s_ref_list[1].curvalue = REF_SOFT;
	}
	else if ( strcmp( r_renderer->string, "gl" ) == 0 )
	{
	...
	for ( i = 0; i < 2; i++ )
	{
		...
		s_brightness_slider[i].curvalue = ( 1.3 - r_gamma->value + 0.5 ) * 10;
