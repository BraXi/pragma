...
static int CM_GridPlane(int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int tri) 
{
	...
	// should never happen
	Com_Printf( "WARNING: %s unresolvable\n", __FUNCTION__);
	return -1;
}
...
static int CM_EdgePlaneNum( cGrid_t *grid, int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int k ) 
{
	...
	Com_Error( ERR_DROP, __FUNCTION__": bad k" );
	return -1;
}
...
static void CM_SetBorderInward( facet_t *facet, cGrid_t *grid, int gridPlanes[MAX_GRID_SIZE][MAX_GRID_SIZE][2], int i, int j, int which ) 
{
	...
	default:
		Com_Error(ERR_FATAL, __FUNCTION__": bad parameter");
		numPoints = 0;
		break;
	}
	...
	for ( k = 0 ; k < facet->numBorders ; k++ ) 
	{
		...
		else 
		{
			...
			Com_Printf( "WARNING: " __FUNCTION__ ": mixed plane sides\n" );
			facet->borderInward[k] = false;
			if ( !debugBlock ) 
			{
				debugBlock = true;
	...
	for ( axis = 0 ; axis < 3 ; axis++ )
	{
		for ( dir = -1 ; dir <= 1 ; dir += 2, order++ )
		{
			...
			if ( i == facet->numBorders ) 
			{
				...
				{
					Com_Printf(__FUNCTION__": too many bevels\n");
				}
			...
			for ( dir = -1 ; dir <= 1 ; dir += 2 )
			{
				...
				if ( i == facet->numBorders ) 
				{
					...
					for (k = 0; k < facet->numBorders; k++)
					{
						...
						{
							Com_Printf(__FUNCTION__":  bevel plane already used\n");
						}
					}
					...
					if (!w2) 
					{
						Com_Printf( __FUNCTION__": invalid bevel\n");
						continue;
					}
					else 
					{
...
struct patchCollide_s *CM_GeneratePatchCollide( int width, int height, vec3_t *points ) 
{
	...
	{
		Com_Error( ERR_DROP, __FUNCTION__": bad parameters: (%i, %i, %p)", width, height, points );
	}

	if ( !(width & 1) || !(height & 1) ) 
	{
		Com_Error( ERR_DROP, __FUNCTION__": even sizes are invalid for quadratic meshes" );
	}

	if ( width > MAX_GRID_SIZE || height > MAX_GRID_SIZE ) 
	{
		Com_Error( ERR_DROP, __FUNCTION__": source is > MAX_GRID_SIZE" );
	}
...
void CM_TracePointThroughPatchCollide( traceWork_t *tw, const struct patchCollide_s *pc ) 
{
	...
	if ( !cm_playerCurveClip->value || !tw->isPoint )
	{
		return;
	}
	...
	for ( i = 0 ; i < pc->numFacets ; i++, facet++ ) 
	{
		...
		if ( j == facet->numBorders ) 
		{
			...
			if (!cv) 
			{
				cv = Cvar_Get( "r_debugSurfaceUpdate", "0", CVAR_CHEAT, NULL ); // ? ? ?
			}
			if (cv->value >= 1.0f) 
			{
...
void CM_DrawDebugSurface( void (*drawPoly)(int color, int numPoints, float *points) ) 
{
	...
	if ( !cv2 )
	{
		cv2 = Cvar_Get( "r_debugSurface", "0", CVAR_CHEAT, NULL );
	}

#if 0
	if (cv2->value != 1.0f)
	{
		BotDrawDebugPolygons(drawPoly, cv2->value);
		return;
	}
#endif
	...
	if ( !cv ) 
	{
		cv = Cvar_Get( "cm_debugSize", "2", CVAR_CHEAT, NULL );
	}
