...
typedef struct
{
	...
	int				serverframe;
	prTime_t /*int*/ servertime;		// server time the message is valid for (in msec)
	int				deltaframe;
...
typedef struct
{
	...
	prTime_t	cmd_time[CMD_BACKUP];	// time sent, for calculating pings

	vec3_t		predicted_origins[CMD_BACKUP];	// for debug comparing against server
	float		predicted_step;				// for stair up smoothing
	prTime_t	predicted_step_time;
	...
	prTime_t	time;			// this is the time value that the clientis rendering at.  
	...
	FILE		*cinematic_file;
	prTime_t	cinematictime;		// cls.realtime for first cinematic frame
	int			cinematicframe;
...
typedef struct
{
	...
	int			framecount;
	prTime_t	realtime;			// always increasing, no clamping, etc
...
#define MAX_FX_STAGES 8 //TODO: eliminate constant, make stages variable sized
#define MAX_FX_TEXTURESTAGES 20 //TODO: eliminate constant, make stages variable sized
typedef struct cparticle_s
{
	qboolean	inuse;

	prTime_t	time; // cl.time the segment was created
	prTime_t	lifeTime; // time in miliseconds after which particle should be removed

	int			type; // 0 = quake, 1 = pragma
	int			flags; // 0:Billboard, 1:Oriented (FX_SetParticleAngles)

	vec3_t		origin;
	vec3_t		velocity; // added to origin, per second
	vec3_t		right, up; // orientation

	struct image_s* tex;

	int			numTextureStages;
	void		*textures[MAX_FX_TEXTURESTAGES]; /* struct image_s* */
	float		texture_times[MAX_FX_TEXTURESTAGES];

	int			numAlphaStages;
	float		alpha_values[MAX_FX_STAGES];
	float		alpha_times[MAX_FX_STAGES];

	int			numSizeStages;
	vec2_t		size_values[MAX_FX_STAGES];
	float		size_times[MAX_FX_STAGES];

	int			numColorStages;
	vec3_t		color_values[MAX_FX_STAGES];
	float		color_times[MAX_FX_STAGES];

	int			numVelocityStages;
	vec3_t		velocity_values[MAX_FX_STAGES];
	float		velocity_times[MAX_FX_STAGES];
