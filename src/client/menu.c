...
void M_Main_Draw (void)
{
	...
	ystart = ( viddef.height - 250 );
	xoffset = 50;// (viddef.width - widest + 70) / 2;


	UI_DrawString(viddef.width / 2, viddef.height / 2, XALIGN_CENTER, va("PRAGMA %s", PRAGMA_VERSION) );
	UI_DrawString(viddef.width / 2, viddef.height / 2 + 12, XALIGN_CENTER, va("build: %s", PRAGMA_TIMESTAMP));
	UI_DrawString(viddef.width / 2, viddef.height / 2 + 24, XALIGN_CENTER, "github.com/braxi/pragma");
...
qboolean PlayerConfig_MenuInit( void )
{
	...
	cvar_t *cl_hand = Cvar_Get( "cl_hand", "1", CVAR_USERINFO | CVAR_ARCHIVE );
	...
	if ( cl_hand->value < 0 || cl_hand->value > 2 )
