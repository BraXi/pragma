...
int CL_ParsePlayerBeam (struct model_s *model)
{
	...
	{
		VectorSet(offset, 2, 7, -3); // PMM - network optimization
	}
...
// find a free beam
	for (i=0, b=cl_playerbeams ; i< MAX_BEAMS ; i++, b++)
	{
		if (!b->model || b->endtime < cl.time)
		{
			b->entity = ent;
			b->model = model;
			b->endtime = cl.time + SV_FRAMETIME_MSEC;		// PMM - this needs to be 100 to prevent multiple heatbeams
			VectorCopy (start, b->start);
			VectorCopy (end, b->end);
			VectorCopy (offset, b->offset);
			return ent;
...
void CL_ParseLaser (int colors)
{
	...
	for (i=0, l=cl_lasers ; i< MAX_LASERS ; i++, l++)
	{
		if (l->endtime < cl.time)
		{
			...
			l->ent.model = NULL;
			l->ent.frame = 4;
			l->endtime = cl.time + SV_FRAMETIME_MSEC;
			return;
		}
	}
}
...
//			Com_Printf ("Sustain effect id %d\n", id);
		...
		if (free_sustain)
		{
			...
			s->thinkinterval = SV_FRAMETIME_MSEC;
			s->nextthink = cl.time;
		}
		else
		{
	...
	for (i=0, b=cl_playerbeams ; i< MAX_BEAMS ; i++, b++)
	{
		...
		if(cl_mod_heatbeam && (b->model == cl_mod_heatbeam))
		{
			...
			{	
				...
				for (j=0 ; j<3 ; j++)
				{
					b->start[j] = cl.refdef.vieworg[j] + ops->viewmodel_offset[j]
						+ cl.lerpfrac * (ps->viewmodel_offset[j] - ops->viewmodel_offset[j]);
				}
...
void CL_AddExplosions (void)
{
	...
	for (i=0, ex=cl_explosions ; i< MAX_EXPLOSIONS ; i++, ex++)
	{
		if (ex->type == ex_free)
			continue;
		frac = (cl.time - ex->start)/SV_FRAMETIME_MSEC;
		f = floor(frac);
...
void CL_ProcessSustain ()
{
	...
	for (i=0, s=cl_sustains; i< MAX_SUSTAINS; i++, s++)
	{
		if (s->id)
			...
			{
//				Com_Printf ("think %d %d %d\n", cl.time, s->v.nextthink, s->thinkinterval);
				s->think (s);
			}
