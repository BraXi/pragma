...
inline void R_Texturing(qboolean newstate)
{
//	if (glstate.textureEnabled[glstate.textureMappingUnit] == newstate)
//		return;

	glstate.textureEnabled[glstate.textureMappingUnit] = newstate;
	if (newstate)
		qglEnable(GL_TEXTURE_2D);
	else
		qglDisable(GL_TEXTURE_2D);
...
inline void R_EnableTextureUnit(int tmu, qboolean newstate)
{
//	if (glstate.textureEnabled[tmu] == newstate)
//		return;
...
void R_BindTextureToCurrentTMU(int texnum)
{
//	if (glstate.textureBind[glstate.textureMappingUnit] == texnum)
//		return;
	R_EnableTextureUnit(glstate.textureMappingUnit, true);
	glstate.textureBind[glstate.textureMappingUnit] = texnum;
	qglBindTexture(GL_TEXTURE_2D, texnum);
}



void R_SelectTextureMappingUnit(int tmu)
{
//	if (tmu == glstate.textureMappingUnit)
//		return;
	glstate.textureMappingUnit = tmu;
	qglActiveTextureARB(textureTargets[tmu]);
}

void R_MultiBindTexture(int tmu, int texnum)
{
	R_SelectTextureMappingUnit(tmu);
//	if (gl_state.currenttextures[tmu] == texnum)
//		return;

	R_BindTextureToCurrentTMU(texnum);
}